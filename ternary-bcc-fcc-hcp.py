...
structures_config = {
    "bcc": {
        ...
        "lattice_constant": 3.2,  # BCC晶格常数 (Å)
    ...
    "fcc": {
        ...
        "lattice_constant": 3.8,  # FCC晶格常数 (Å)
...
# 创建输出目录
output_dir = "ternary_alloys_multistructure"
...
def generate_fcc_positions(size_x, size_y, size_z):
    ...
    for i in range(size_x):
        for j in range(size_y):
            for k in range(size_z):
                positions.append([i, j, k])               # 角点
                positions.append([i + 0.5, j + 0.5, k])    # 面心
                positions.append([i + 0.5, j, k + 0.5])    # 面心
                positions.append([i, j + 0.5, k + 0.5])    # 面心
    return np.array(positions)
...
# 将分数坐标转换为直角坐标
def fractional_to_cartesian(frac_pos, structure_type, lattice_const):
    if structure_type == "hcp":
        ...
        # 超胞尺寸
        supercell_size = structures_config["hcp"]["supercell_size"]
...
# 生成三元合金结构
def generate_ternary_alloy(elements, concentrations, positions, structure_type):
    # 检查浓度有效性
    ...
    # 转换为直角坐标
    if structure_type == "hcp":
        cart_positions, cell_sizes = fractional_to_cartesian(
            positions, structure_type, config["lattice_constant"]
        )
    else:
        cart_positions, cell_sizes = fractional_to_cartesian(
            positions, structure_type, config["lattice_constant"]
    ...
    xyz_content += f"Ternary={'-'.join(elements)} "
...
def generate_sparse_concentration_combinations():
    ...
    values = [round(i * concentration_step, 2) for i in 
              range(int(min_concentration / concentration_step), 
                    int((1.0 - 2*min_concentration) / concentration_step) + 1)]
...
if __name__ == "__main__":
    # 生成所有可能的三元组合
    element_combinations = list(combinations(elements, 3))
    ...
    for config in structures_config.values():
        supercell = config["supercell_size"]
        total_atoms = supercell[0] * supercell[1] * supercell[2] * config["atoms_per_unit"]
        structures_per_type = len(element_combinations) * len(concentration_combinations)
    ...
    # 创建总进度条
    pbar_total = total_structures_all
    pbar = tqdm(total=pbar_total, desc="Generating all ternary structures")
    ...
    for structure_type, config in structures_config.items():
        ...
        for trio in element_combinations:
            trio_dir = os.path.join(struct_output_dir, f"{'_'.join(trio)}")
            os.makedirs(trio_dir, exist_ok=True)
            
            for conc in concentration_combinations:
                concentrations = list(conc)
                xyz_data = generate_ternary_alloy(
                    trio, concentrations, positions, structure_type
                )
                
                if xyz_data:
                    # 创建文件名
                    conc_str = '_'.join([f"{c:.2f}" for c in concentrations]).replace('.', 'p')
                    filename = f"{'_'.join(trio)}_{structure_type}_c_{conc_str}.xyz"
                    filepath = os.path.join(trio_dir, filename)
    ...
    print(f"\nNumber of element trios: {len(element_combinations)}")
    ...
    for structure_type in structures_config.keys():
        ...
        print(f"      element1_element2_element3/")
