...
concentration_step = 0.1  # 浓度变化间隔
min_concentration = 0.1   # 最小浓度
sampling_step = 1         # 采样步长

# 创建输出目录
output_dir = "quinary_alloys_multistructure"  # 改为五元合金
...
# 生成五元合金结构（修改为五元）
def generate_quinary_alloy(elements, concentrations, positions, structure_type):
    # 检查浓度有效性
    ...
    xyz_content += f"Quinary={'-'.join(elements)} "  # 改为五元
...
def generate_sparse_concentration_combinations():
    ...
    values = [round(i * concentration_step, 2) for i in 
              range(int(min_concentration / concentration_step), 
                    int((1.0 - 4*min_concentration) / concentration_step) + 1)]  # 4个最小浓度
    ...
    for c1 in sparse_values:
        for c2 in sparse_values:
            for c3 in sparse_values:
                for c4 in sparse_values:
                    c5 = 1.0 - c1 - c2 - c3 - c4
                    if c5 >= min_concentration:
                        concentrations.append((c1, c2, c3, c4, c5))
...
if __name__ == "__main__":
    # 生成所有可能的五元组合（修改为5）
    element_combinations = list(combinations(elements, 5))
    ...
    # 创建总进度条
    pbar = tqdm(total=total_structures_all, desc="Generating all quinary structures")  # 改为五元
    ...
    for structure_type, config in structures_config.items():
        ...
        for pentad in element_combinations:  # 改为五元
            pentad_dir = os.path.join(struct_output_dir, f"{'_'.join(pentad)}")
            os.makedirs(pentad_dir, exist_ok=True)
            
            for conc in concentration_combinations:
                concentrations = list(conc)
                xyz_data = generate_quinary_alloy(  # 改为五元函数
                    pentad, concentrations, positions, structure_type
                )
                
                if xyz_data:
                    # 创建文件名
                    conc_str = '_'.join([f"{c:.2f}" for c in concentrations]).replace('.', 'p')
                    filename = f"{'_'.join(pentad)}_{structure_type}_c_{conc_str}.xyz"
                    filepath = os.path.join(pentad_dir, filename)
    ...
    print(f"  Number of element pentads: {len(element_combinations):,}")  # 改为五元
    ...
    for structure_type in structures_config.keys():
        ...
        print(f"      element1_element2_element3_element4_element5/")  # 改为5个元素
        print(f"        *_{structure_type}_c_*.xyz files")
    
    print("\nNote: This script generates QUINARY alloy structures with")  # 改为五元
